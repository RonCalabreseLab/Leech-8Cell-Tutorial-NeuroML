...
PARAMETER {
    ...
    m_instances = 3 
    m_timeCourse_offset = 3.861 (ms)
    m_timeCourse_rate = -3.434 (ms)
    m_timeCourse_midpoint = -51.350002 (mV)
    m_timeCourse_scale = 5.98 (mV)
    m_steadyState_rate = 1 
    m_steadyState_midpoint = -29.130001 (mV)
    m_steadyState_scale = 8.922001 (mV)
    h_instances = 1 
    h_timeCourse_offset = 2.8339999 (ms)
    h_timeCourse_rate = -2.3709998 (ms)
    h_timeCourse_midpoint = -21.9 (mV)
    h_timeCourse_scale = 2.641 (mV)
    h_steadyState_rate = 1 
    h_steadyState_midpoint = -40 (mV)
    h_steadyState_scale = -6.048 (mV)
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=naChan type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = m_timeCourse_offset  +  m_timeCourse_rate  / (1 + exp(0 - (v -  m_timeCourse_midpoint )/ m_timeCourse_scale )) ? evaluable
    ...
    h_timeCourse_t = h_timeCourse_offset  +  h_timeCourse_rate  / (1 + exp(0 - (v -  h_timeCourse_midpoint )/ h_timeCourse_scale )) ? evaluable
