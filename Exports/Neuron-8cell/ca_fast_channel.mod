TITLE Mod file for component: Component(id=ca_fast_channel type=ionChannelHH)
...
NEURON {
    ...
    RANGE m_timeCourse_v_cosh               : derived variable
...
PARAMETER {
    ...
    conductance = 0.0241 (uS)
    m_instances = 2 
    m_steadyState_rate = 1 
    m_steadyState_midpoint = -46.7 (mV)
    m_steadyState_scale = 1.666667 (mV)
    m_timeCourse_offset = 0.011 (ms)
    m_timeCourse_rate = 0.024 (ms)
    m_timeCourse_midpoint = -46.7 (mV)
    m_timeCourse_scale = 3.0303 (mV)
    h_instances = 1 
    h_steadyState_rate = 1 
    h_steadyState_midpoint = -55.5 (mV)
    h_steadyState_scale = -2.85714 (mV)
    h_timeCourse_offset = 0.06 (ms)
    h_timeCourse_rate = 0.31 (ms)
    h_timeCourse_midpoint = -55.5 (mV)
    h_timeCourse_scale = -3.37 (mV)
...
ASSIGNED {
    ...
    m_timeCourse_v_cosh                    : derived variable
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=ca_fast_channel type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=ca_fast_channel type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_v_cosh = -(v -  m_timeCourse_midpoint )/ m_timeCourse_scale ? evaluable
    m_timeCourse_t = m_timeCourse_offset  +  m_timeCourse_rate  / ((exp( m_timeCourse_v_cosh ) + exp(0 -  m_timeCourse_v_cosh ))/ 2) ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=tauMCaF)
