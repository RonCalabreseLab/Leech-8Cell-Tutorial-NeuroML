TITLE Mod file for component: Component(id=h_channel type=ionChannelHH)
...
NEURON {
    ...
    RANGE m_steadyState_scale_1             : parameter
    RANGE m_steadyState_scale_2             : parameter
...
PARAMETER {
    ...
    m_steadyState_scale_1 = 5.555555 (mV)
    m_steadyState_scale_2 = 2 (mV)
    m_steadyState_scale = 0 (mV)
    m_timeCourse_offset = 0.7 (ms)
    m_timeCourse_rate = 1.7 (ms)
    m_timeCourse_midpoint = -73 (mV)
    m_timeCourse_scale = 10 (mV)
...
ASSIGNED {
    ...
    m_steadyState_x : no units???          : derived variable
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=h_channel type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=h_channel type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_steadyState_x = m_steadyState_rate  / (1 + (2 * exp(( v +  m_steadyState_midpoint )/ m_steadyState_scale_1 )) + exp((v +  m_steadyState_midpoint )/ m_steadyState_scale_2 )) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=m type=gateHHtauInf), from steadyState; Component(id=null type=hSteadyState)
