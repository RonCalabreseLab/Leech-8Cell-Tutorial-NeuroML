TITLE Mod file for component: Component(id=k_fast_channel type=ionChannelHH)
...
NEURON {
    ...
    RANGE m_timeCourse_VOLT                 : parameter
...
PARAMETER {
    ...
    m_timeCourse_midpoint = 40 (mV)
    m_timeCourse_scale = 10 (mV)
    m_timeCourse_SEC = 1000 (ms)
    m_timeCourse_VOLT = 1000 (mV)
}
...
ASSIGNED {
    ...
    m_timeCourse_t0 (ms)                   : derived variable
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=k_fast_channel type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=k_fast_channel type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t0 = (1.5 + (8.0 / 1 + exp(0 - (v + 0.022* m_timeCourse_VOLT )/ m_timeCourse_scale )) + (-2.2 / ((exp( m_timeCourse_v_cosh ) + exp(0 -  m_timeCourse_v_cosh )) / 2)))* m_timeCourse_SEC ? evaluable
