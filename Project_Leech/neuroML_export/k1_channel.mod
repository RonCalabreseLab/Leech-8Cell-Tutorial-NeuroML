TITLE Mod file for component: Component(id=k1_channel type=ionChannelHH)
...
PARAMETER {
    ...
    m_steadyState_midpoint = 21 (mV)
    m_steadyState_scale = -6.993007 (mV)
    m_timeCourse_offset = 1 (ms)
    m_timeCourse_rate = 11 (ms)
    m_timeCourse_midpoint = 16 (mV)
    m_timeCourse_scale = 6.6667 (mV)
    h_instances = 1 
    h_steadyState_rate = 1 
    h_steadyState_midpoint = 28 (mV)
    h_steadyState_scale = 9.009 (mV)
    h_timeCourse_offset = 500 (ms)
    h_timeCourse_rate = 200 (ms)
    h_timeCourse_midpoint = 13 (mV)
    h_timeCourse_scale = -6.993007 (mV)
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=k1_channel type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=k1_channel type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = m_timeCourse_offset  +  m_timeCourse_rate  / (1 + exp((v +  m_timeCourse_midpoint )/ m_timeCourse_scale )) ? evaluable
    ...
    h_timeCourse_t = h_timeCourse_offset  +  h_timeCourse_rate  / (1 + exp((v +  h_timeCourse_midpoint )/ h_timeCourse_scale )) ? evaluable
